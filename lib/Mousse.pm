# This file was generated by Mousse::Maker 0.06 from Mouse 0.65.
#
...
use warnings FATAL => 'redefine'; # to avoid to load Mousse::PurePerl
...
sub new {
	...
	my $meta = Mousse::Meta::Class->initialize($class);
...
sub do_import {
	...
	my $spec = $SPEC{$package}
		|| confess("The package $package package does not use Mousse::Exporter");
...
sub do_unimport {
	...
	my $spec = $SPEC{$package}
		|| confess("The package $package does not use Mousse::Exporter");
...
BEGIN{
	...
	# XS loader is placed here, not in Mousse.pm.

	our $VERSION = '0.65';

	my $xs = !(exists $INC{'Mousse/PurePerl.pm'} || $ENV{MOUSE_PUREPERL} || $ENV{PERL_ONLY});
...
sub apply_all_roles {
	my $consumer = Scalar::Util::blessed($_[0])
		?                                shift   # instance
		: Mousse::Meta::Class->initialize(shift); # class or role name
...
use overload
	...
	'|'      => sub {                  # or-combination
		...
		return Mousse::Util::TypeConstraints::find_or_parse_type_constraint(
			"$_[0] | $_[1]",
		);
	},
...
sub _create_type{
	...
	if($mode eq 'subtype'){
		...
		$constraint = Mousse::Meta::TypeConstraint->new(%args);
	}
...
sub find_type_constraint {
	my($spec) = @_;
	return $spec if Mousse::Util::is_a_type_constraint($spec);
...
sub _canonicalize_handles {
	...
	if (ref($handles) eq 'HASH') {
		...
		return $handles->( $self, Mousse::Meta::Class->initialize("$class_or_role"));
	}
	elsif (ref($handles) eq 'Regexp') {
		my $class_or_role = ($self->{isa} || $self->{does})
			|| $self->throw_error("Cannot delegate methods based on a Regexp without a type constraint (isa)");

		my $meta = Mousse::Meta::Class->initialize("$class_or_role"); # "" for stringify
		return map  { $_ => $_ }
			   grep { !Mousse::Object->can($_) && $_ =~ $handles }
...
sub throw_error{
	...
	my $metaclass = (ref $self && $self->associated_class) || 'Mousse::Meta::Class';
...
sub _install_modifier {
	...
	if($no_cmm_fast){
		...
		$impl = sub {
			...
			my $method = Mousse::Util::get_code_ref( $into, $name );
			...
			if ( !$method || !Data::Util::subroutine_modifier($method) ) {
				...
				$self->add_method($name => Mousse::Util::get_code_ref($into, $name));
			}
...
sub _generate_destructor{
	...
	for my $class ($metaclass->linearized_isa) {
		if (Mousse::Util::get_code_ref($class, 'DEMOLISH')) {
			$demolishall .= sprintf "%s::DEMOLISH(\$self, \$Mousse::Util::in_global_destruction);\n",
				$class,
		}
	}
...
sub _metaclass_cache { # DEPRECATED
	my($self, $name) = @_;
	Carp::cluck('_metaclass_cache() has been deprecated. Use Mousse::Util::get_metaclass_by_name() instead');
	return $METAS{$name};
}
...
sub _collect_methods { # Mousse specific
	my($meta, @args) = @_;
...
sub apply {
	...
	if(Mousse::Util::is_a_metaclass($consumer)){  # Application::ToClass
		...
		$consumer = (Mousse::Util::class_of($instance) || 'Mousse::Meta::Class')->create_anon_class(
...
sub combine {
	...
	require 'Mousse/Meta/Role/Composite.pm'; # we don't want to create its namespace

	my $composite = Mousse::Meta::Role::Composite->create_anon_role();
...
sub _apply_methods{
	...
	if(exists $self->{conflicting_methods}){
		...
		if(@conflicting == 1){
			...
			my $roles       = Mousse::Util::quoted_english_list(map{ $_->name } @{ $self->{composed_roles_by_method}{$method_name} });
			$self->throw_error(
...
use Mousse::Util  qw(not_supported);
use Mousse::Meta::Role;
use Mousse ();
...
sub with     {
	...
	Mousse::Util::apply_all_roles($meta->name, @_);
	return;
}
